// app/accounts/vouchers/payment/cash/submit/page.tsx
"use client";
...
export default function CashPaymentVoucherPage() {
  ...
  const [sourceId, setSourceId] = useState(""); // party (payee)
  const [modeId, setModeId] = useState("");
  const [reference, setReference] = useState("");
  const [narration, setNarration] = useState("");
  const [selectedCashVaId, setSelectedCashVaId] = useState("");

  /* Debit rows (what we're paying) */
  ...
  /* Supplier search state (if party is supplier) */
  ...
  /* Cash accounts – role Cash, active, and allowed CashPayment */
  const cashVas = useMemo(() => {
    return voucherAccounts.filter((v) => {
      if (!v || v.isActive === false) return false;
      if (v.role !== "Cash") return false;
      const types = v.allowedVoucherTypes || v.voucherTypes || [];
      return types.length === 0 || types.includes("CashPayment") || types.includes("All");
    });
  }, [voucherAccounts]);

  /* Ledger options for debit lines – only CashPayment/All, not Cash itself */
  const ledgerOptions = useMemo(() => {
    return voucherAccounts.filter((va) => {
      if (va.isActive === false) return false;
      if (va.role === "Cash") return false; // exclude cash
      if (!va.allowedVoucherTypes || va.allowedVoucherTypes.length === 0) return true;
      return (
        va.allowedVoucherTypes.includes("CashPayment") ||
        va.allowedVoucherTypes.includes("All")
      );
    });
  }, [voucherAccounts]);

  /* Parties – Payment direction, active, CashPayment */
  ...
  /* Payment modes – only CashPayment */
  ...
  const selectedCashVa = useMemo(
    () => cashVas.find((c) => c._id === selectedCashVaId) || null,
    [cashVas, selectedCashVaId],
  );

  const cashAccountNode = useMemo(() => {
    if (!selectedCashVa?.accountId) return null;
    const accId =
      typeof selectedCashVa.accountId === "object"
        ? (selectedCashVa.accountId as any)._id
        : selectedCashVa.accountId;
    return accountsMap.get(String(accId));
  }, [selectedCashVa, accountsMap]);

  const cashOpeningBalance = useMemo(() => {
    if (!cashAccountNode) return 0;
    return (
      Number(cashAccountNode.openingBalance ?? 0) +
      Number(cashAccountNode.periodDebit ?? 0) -
      Number(cashAccountNode.periodCredit ?? 0)
    );
  }, [cashAccountNode]);

  const closingBalance = useMemo(
    () => cashOpeningBalance - totalDebits,
    [cashOpeningBalance, totalDebits],
  );
    ...
    (async () => {
      ...
      try {
        const [accountsRes, partiesRes, modesRes, vasRes] = await Promise.all([
          ...
          api.get("/voucher-parties?voucherTypes=CashPayment").catch(() => ({ data: [] })),
          api.get("/payment-modes?voucherTypes=CashPayment").catch(() => ({ data: [] })),
          api.get("/voucher-accounts?allowedVoucherTypes=CashPayment").catch(() => ({ data: [] })),
        ]);
        if (!mounted) return;
  ...
  useEffect(() => {
    if (!selectedCashVaId && cashVas.length > 0) {
  ...
  async function submitVoucher() {
    if (!selectedCashVaId) return toast.error("Select a Cash ledger.");
    ...
    if (!cashVa?.accountId) return toast.error("Cash ledger not mapped to COA.");

    const lines: any[] = [
      // Cash credit (since we're paying out of cash)
      {
        accountId: cashVa.accountId,
        debit: 0,
        credit: Number(totalDebits),
        narration: `Cash payment ${reference ? `(Ref: ${reference})` : ""}`.trim(),
      },
    ];
    ...
    const payload = {
      ...
      lines,
      status: "Pending",
      bankVaId: selectedCashVaId, // backend reuses bankVaId for cash VA id
      type: "CashPayment",
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Record a cash payment — select payee, mode, cash ledger and debit accounts.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-4 shadow-sm border-gray-200 bg-white">
          <Label className="text-xs text-gray-500">Cash Ledger (Credit)</Label>
          <Select
            value={selectedCashVaId}
            onValueChange={(v) => setSelectedCashVaId(v || "")}
          >
            <SelectTrigger className="mt-1 h-9 border-gray-200">
            ...
            <SelectContent>
              {cashVas.map((c) => {
                const mappedAcc = c.accountId
                  ? accountsMap.get(String(c.accountId))
                  : undefined;
                const label =
                  (c.name || "") +
          ...
          <p className="text-xs text-gray-400 mt-2">
            Only cash‑type voucher accounts are shown.
          </p>
        </Card>
