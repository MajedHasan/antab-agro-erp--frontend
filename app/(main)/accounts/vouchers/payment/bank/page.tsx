// app/accounts/vouchers/payment/bank/page.tsx
"use client";
...
import {
  buildBankPaymentVoucherHtml,
  buildVoucherPrintHeader,
  BankPaymentVoucherPreview,
} from "./_components/BankPaymentVoucherContent";
...
/* ---------- component ---------- */
export default function BankPaymentVoucherListPage() {
  ...
  /* Masters – filtered for BankPayment */
  useEffect(() => {
    loadMasters();
  }, []);
  ...
  async function loadMasters() {
    try {
      const [vasRes, partiesRes] = await Promise.all([
        ...
        api.get("/voucher-parties?voucherTypes=BankPayment").catch(() => ({ data: { data: [] } })),
      ]);
  ...
  const computeTotals = (lines: VoucherLine[]) => {
    let gross = 0;
    lines.forEach((l) => {
      gross += l.debit || 0; // payments: debits are the amounts paid out
    });
    return { gross, net: gross }; // for simplicity
  };

  const getPrintHtml = () => {
    if (!selectedVoucher) return "";
    return buildBankPaymentVoucherHtml(
      {
  ...
  const exportCSV = async () => {
    try {
      ...
      a.href = url;
      a.download = `bank-payments-${new Date().toISOString().slice(0, 10)}.csv`;
      a.click();
  ...
  return (
    <div className="max-w-[1500px] mx-auto p-4 md:p-6 space-y-5">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Bank Payment Vouchers</h1>
          <p className="text-sm text-gray-500 mt-1">Manage bank payment vouchers — filter, view, and approve.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button onClick={() => router.push("/accounts/vouchers/payment/bank/submit")}>
            + Add Voucher
          </Button>
        </div>
      </div>
            ...
            <TableBody>
              {loading ? (
                ...
                data.map((v) => {
                  ...
                  return (
                    <TableRow key={v._id} className="hover:bg-blue-50/30 transition-colors">
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          ...
                          {status === "Approved" && (
                            <GlobalPrintButton
                              contentHtml={buildBankPaymentVoucherHtml(
                                {
