// app/accounts/vouchers/receive/cash/page.tsx
"use client";
...
import {
  buildCashReceiveVoucherHtml,
  buildVoucherPrintHeader,
  CashReceiveVoucherPreview,
} from "./_components/CashReceiveVoucherContent";
...
export default function CashReceiveVoucherListPage() {
  ...
  /* Masters – filtered for CashReceive */
  useEffect(() => {
    loadMasters();
  }, []);
  ...
  async function loadMasters() {
    try {
      const [vasRes, partiesRes] = await Promise.all([
        ...
        api.get("/voucher-parties?voucherTypes=CashReceive").catch(() => ({ data: { data: [] } })),
      ]);
  ...
  const computeGrossAndDiscount = (lines: VoucherLine[]) => {
    let gross = 0,
      discount = 0;
    lines.forEach((l) => {
      gross += l.credit || 0;
      const narr = (l.narration || "").toLowerCase();
      if (/discount/.test(narr)) discount += l.debit || 0;
    });
  ...
  const getPrintHtml = () => {
    ...
    return buildCashReceiveVoucherHtml(
      {
  ...
  const exportCSV = async () => {
    // similar to before but with proper fields, omitted for brevity
  ...
  return (
    <div className="max-w-[1500px] mx-auto p-4 md:p-6 space-y-5">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Cash Receive Vouchers</h1>
          <p className="text-sm text-gray-500 mt-1">Manage cash receipt vouchers — filter, view, and approve.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button onClick={() => router.push("/accounts/vouchers/receive/cash/submit")}>
            + Add Voucher
          </Button>
        </div>
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                data.map((v) => {
                  const lines = v.lines || [];
                  const { gross, discount, net } = computeGrossAndDiscount(lines);
                  ...
                  return (
                    <TableRow key={v._id} className="hover:bg-green-50/30 transition-colors">
                      ...
                      <TableCell className="text-right text-sm">{formatTaka(discount)}</TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          ...
                          {status === "Approved" && (
                            <GlobalPrintButton
                              contentHtml={buildCashReceiveVoucherHtml(
                                {
