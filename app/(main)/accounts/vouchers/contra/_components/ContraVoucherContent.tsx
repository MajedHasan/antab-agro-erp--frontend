// components/print/ContraVoucherContent.tsx
...
export function buildContraVoucherHtml(
  voucher: VoucherData,
  lines: VoucherLine[],
) {
...
">
  ...
  <table style="width:100%; margin-bottom:16px; border-collapse:collapse;">
    <tr>
      ...
      <td style="width:35%; text-align:right; vertical-align:top;">
        <div style="font-size:18px; font-weight:700; color:#0f172a; text-transform:uppercase; letter-spacing:1px;">Contra Voucher</div>
  ...
  <table style="width:100%; margin-bottom:16px; background:#f8fafc; padding:10px 16px; border-radius:4px;">
    <tr>
      <td style="width:50%; padding:4px 0;">
        <div style="font-size:9px; color:#64748b; text-transform:uppercase;">From Account (Credit)</div>
        <div style="font-weight:500;">${voucher.fromAccount || "—"}</div>
      </td>
      <td style="width:50%; padding:4px 0;">
        <div style="font-size:9px; color:#64748b; text-transform:uppercase;">To Account (Debit)</div>
        <div style="font-weight:500;">${voucher.toAccount || "—"}</div>
      </td>
    </tr>
    <tr>
  ...
  <!-- Ledger Table (optional, shows lines) -->
  <table style="width:100%; border-collapse:collapse; margin-bottom:16px;">
    <thead>
      <tr style="background:#e2e8f0; font-size:10px; text-transform:uppercase; color:#334155;">
        <th style="text-align:left; padding:6px 8px; border-bottom:2px solid #94a3b8;">Account</th>
...
export function ContraVoucherPreview({
  voucher,
  lines,
}: {
  voucher: VoucherData;
    ...
    <div className="border-2 border-slate-800 p-6 md:p-8 max-w-[210mm] mx-auto bg-white text-slate-800 shadow-sm">
      ...
      <div className="flex justify-between mb-4">
        ...
        <div className="text-right">
          <div className="text-lg font-bold text-slate-800 uppercase tracking-wide">Contra Voucher</div>
      ...
      <div className="flex gap-4 mb-4 bg-slate-100 rounded p-3">
        <div className="flex-1">
          <div className="text-[9px] text-slate-500 uppercase">From Account (Credit)</div>
          <div className="font-medium text-sm">{voucher.fromAccount || "—"}</div>
        </div>
        <div className="flex-1">
          <div className="text-[9px] text-slate-500 uppercase">To Account (Debit)</div>
          <div className="font-medium text-sm">{voucher.toAccount || "—"}</div>
        </div>
        <div className="flex-1">
      ...
      <table className="w-full border-collapse mb-4">
        <thead>
          <tr className="bg-slate-200 text-[10px] uppercase text-slate-600">
            <th className="text-left px-3 py-2 border-b-2 border-slate-300">Account</th>
