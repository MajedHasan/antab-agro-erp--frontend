...
export default function WarehouseTransferActionPage() {
    ...
    if (key === "finalQty") {
      ...
      item.qtyHistory = [
        ...(item.qtyHistory || []),
        {
          ...
          note: "Edited on frontend before submit",
        },
      ];
    }
  ...
  async function receiverApprove() {
    if (!transfer) return;
    try {
      setSubmitting(true);
      const res = await api.post(`/transfers/${id}/receiver-nsm-approve`, {
        items: actionPayload(),
        remarks: remarks || undefined,
      });
      toast.success("Receiver NSM approved");
      setTransfer(res.data?.data || transfer);
      setRemarks("");
    } catch (err: any) {
  ...
  async function senderReview() {
    if (!transfer) return;
    try {
      setSubmitting(true);
      const res = await api.post(`/transfers/${id}/sender-review`, {
        items: actionPayload(),
        remarks: remarks || undefined,
      });
      toast.success("Sender reviewed");
      setTransfer(res.data?.data || transfer);
      setRemarks("");
    } catch (err: any) {
  ...
  async function senderNsmApprove() {
    if (!transfer) return;
    try {
      setSubmitting(true);
      const res = await api.post(`/transfers/${id}/sender-nsm-approve`, {
        items: actionPayload(),
        remarks: remarks || undefined,
      });
      toast.success("Sender NSM approved");
      setTransfer(res.data?.data || transfer);
      setRemarks("");
    } catch (err: any) {
  ...
  async function generateSnapshot() {
    ...
    try {
      setSubmitting(true);
      const res = await api.post(`/transfers/${id}/print-snapshot`);
      toast.success("Print snapshot generated");
      setTransfer(res.data?.data || transfer);
    } catch (err: any) {
      toast.error(err?.response?.data?.message || "Snapshot generation failed");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function uploadSignedDocumentOnly() {
    if (!signedFile) {
      toast.error("Select a signed file first");
      return;
    }
    ...
    try {
      ...
      const res = await api.post(
        "/media/upload?module=warehouse-transfer&folder=signed-documents",
        formData,
        {
          headers: {
  ...
  const workflowCount = workflowLabels.length;
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-slate-300">
                {isWarehouseToWarehouseTransfer(transfer)
                  ? "Warehouse workflow"
                  : isFactoryDirectTransfer(transfer)
                    ? "Factory direct workflow"
                    : "Factory request workflow"}{" "}
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-300">
                ...
                <span className="rounded-full border border-white/15 bg-white/10 px-3 py-1">
                  Step {Math.min(stepIndex + 1, workflowCount)} /{" "}
                  {workflowCount}
                </span>
                <span className="rounded-full border border-white/15 bg-white/10 px-3 py-1">
                  {statusMeta.hint}
          ...
          <CardContent>
            <div className="w-full overflow-x-auto">
              <div
                ...
                style={{
                  minWidth: `${Math.max(workflowCount * 150, 720)}px`,
                  gridTemplateColumns: `repeat(${workflowCount}, minmax(0, 1fr))`,
                }}
              >
            ...
            <Card className="shadow-sm">
              ...
              <CardContent>
                ...
                {editable ? (
                  ...
                  <div className="mt-4 rounded-2xl border bg-slate-50 p-4 text-sm text-muted-foreground">
                    Quantity is locked for this stage.
                  </div>
                )}
              </CardContent>
            </Card>
              ...
              <Card className="shadow-sm">
                ...
                <CardContent className="space-y-3">
                  <textarea
                    ...
                    placeholder="Optional remarks for approval / review"
                  />
                  <div className="text-xs text-muted-foreground">
                    These remarks will be attached to approval workflow actions.
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="space-y-3">
                  {transfer.status === "DRAFT" && (
                    <>
                      ...
                      {canApproveReceiver(transfer) && (
                        <Button
                          className="w-full"
                          onClick={receiverApprove}
                          disabled={submitting}
                        >
                          Receiver NSM approve
                        </Button>
                      )}
                  ...
                  {transfer.status === "RECEIVER_NSM_APPROVED" && (
                    <>
                      ...
                      {canDispatch(transfer) &&
                        isFactoryRequestTransfer(transfer) && (
                          <Button
                            className="w-full"
                            onClick={dispatchTransfer}
                            disabled={submitting}
                  ...
                  {transfer.status === "SENDER_NSM_APPROVED" && (
                    <>
                      <Button
                        ...
                        disabled={submitting || !canPrint(transfer)}
                      >
                        <FileText className="mr-2 h-4 w-4" />
                        Generate print snapshot
                      </Button>

                      <Button
                        className="w-full"
                        onClick={dispatchTransfer}
                        disabled={submitting || !canDispatch(transfer)}
                      >
                        Dispatch transfer
                      </Button>
                    </>
                      ...
                      <div className="rounded-2xl border bg-slate-50 p-4">
                        ...
                        <div className="mt-3 space-y-3">
                          ...
                          <Button
                            className="w-full"
                            variant="outline"
                            onClick={uploadSignedDocumentOnly}
                            disabled={!signedFile || signedUploadLoading}
                          >
                  ...
                  {transfer.status === "COMPLETED" && (
                    <div className="rounded-2xl border bg-emerald-50 p-4 text-sm text-emerald-900">
                      This transfer is completed. No more workflow actions are
                      available.
                    </div>
                  )}
