...
export default function TadaRateEditPage() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get(`/tada/rates/${id}`);
      const data: RateRecord = res.data?.data || res.data;

      setForm({
        rateType: data.rateType,
        takaPerKm: String(data.takaPerKm ?? ""),
        defaultDA: String(data.defaultDA ?? 0),
        defaultNH: String(data.defaultNH ?? 0),
        applicableTo:
          typeof data.applicableTo === "string"
            ? data.applicableTo
            : data.applicableTo?._id || data.applicableTo?.name || "",
        effectiveFrom: data.effectiveFrom
          ? new Date(data.effectiveFrom).toISOString().slice(0, 10)
          : todayInputValue(),
        effectiveTo: data.effectiveTo
          ? new Date(data.effectiveTo).toISOString().slice(0, 10)
          : "",
      ...
      toast.error(
        err?.response?.data?.message || err?.message || "Failed to load rate.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.put(`/tada/rates/${id}`, payload);
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-3xl p-4 md:p-6">
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardHeader>
            <CardTitle>Edit TADA Rate</CardTitle>
          </CardHeader>
