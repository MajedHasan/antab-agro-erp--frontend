...
export default function PackagingMaterialsPage() {
  ...
  const [selectedStockForTx, setSelectedStockForTx] =
    useState<PackagingStock | null>(null);
  ...
  async function fetchStockTransactions(stock: PackagingStock) {
  ...
  async function submitForm(e?: React.FormEvent) {
    ...
    try {
      if (editing?._id) {
        ...
        toast.success("Updated packaging item");
      } else {
        await api.post("/packaging-items", form);
        toast.success("Created packaging item");
      }
      setOpenForm(false);
      fetchList();
    } catch (err: any) {
  ...
  async function deleteItem(id?: string) {
    if (!id || !confirm("Delete packaging item?")) return;
    try {
  ...
  async function toggleActive(item: PackagingItem) {
    if (!item._id) return;
    try {
      const newVal = !item.isActive;
      await api.put(`/packaging-items/${item._id}`, { isActive: newVal });
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage packaging items, stock levels, and transactions across
            factories.
          </p>
        </div>
              ...
              <div className="grid gap-4">
                {(stockByItem.get(selectedItem?._id || "")?.rows || []).map(
                  (s) => {
                    ...
                    const reorderLevel = minLevel > 0 ? minLevel * 2 : 100; // fallback if only min defined
      ...
      <Dialog open={adjustOpen} onOpenChange={setAdjustOpen}>
        <DialogContent>
          <form onSubmit={submitAdjust} className="space-y-4">
            <h3 className="text-xl font-bold flex items-center gap-2">
              <BarChart3 className="w-5 h-5" /> Adjust Packaging Stock
            </h3>
            <div>
