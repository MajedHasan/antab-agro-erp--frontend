 ...
 * - Matches your packaging.model.ts
 * - Endpoints:
 *    GET/POST/PUT/DELETE /packaging-items
 *    GET/POST/PUT /packaging-stocks
 *    GET /warehouses?type=Factory
 *
 * UX:
 * - unit is a select (only "pcs" per model)
 * - adjust stock modal auto-fills existing stock for (item, factory)
 * - factory selector searchable
 */
...
type PackagingStock = {
  _id?: string;
  packagingItemId: string | { _id: string; name?: string };
...
export default function PackagingMaterialsPage() {
  ...
  const [form, setForm] = useState<PackagingItem>({
    name: "",
    sku: "",
    unit: "pcs",
    purchasePrice: 0,
  ...
  // Stocks
  const [stocks, setStocks] = useState<PackagingStock[]>([]);
  ...
  const [adjustRecord, setAdjustRecord] = useState<
    Partial<PackagingStock> & { amount?: number }
  >({});

  // Factories list (for selection)
  const [factories, setFactories] = useState<{ _id: string; name: string }[]>(
    []
  );
  const [factoryFilter, setFactoryFilter] = useState("");

  // small helper: normalize id from populated response
  function idOf(x: any) {
    if (!x) return "";
  ...
  async function fetchStocks() {
    try {
      ...
      const res = await api.get("/packaging-stocks", {
        params: { page: 1, limit: 10000 },
      });
      setStocks(res.data.data || []);
    } catch (err) {
      console.error(err);
      toast.error("Failed to load packaging stocks");
    } finally {
  ...
  const stockByItem = useMemo(() => {
    const map = new Map<string, { total: number; rows: PackagingStock[] }>();
  ...
  // map item+factory -> stock
  const stockByItemFactory = useMemo(() => {
    const map = new Map<string, PackagingStock>();
    for (const s of stocks) {
      const pid = idOf(s.packagingItemId);
  ...
  async function submitForm(e?: React.FormEvent) {
    ...
    try {
      if (editing?._id) {
        await api.put(`/packaging-items/${editing._id}`, form);
        toast.success("Updated");
      } else {
        await api.post("/packaging-items", form);
  ...
  async function deleteItem(id?: string) {
    if (!id) return;
    if (!confirm("Delete packaging item?")) return;
    try {
      await api.delete(`/packaging-items/${id}`);
  ...
  // Stock actions
  function openStockModal(item: PackagingItem) {
    setSelectedItem(item);
    setStockModalOpen(true);
  }

  // open adjust modal; when opened from a stock row provide stock to prefill, otherwise from item
  function openAdjustModal(stock?: PackagingStock, item?: PackagingItem) {
    const itemId = stock ? idOf(stock.packagingItemId) : item?._id;
  ...
  // when user selects factory in adjust modal - prefill existing stock if present
  function handleAdjustFactoryChange(factoryId: string) {
    const key = `${adjustRecord.packagingItemId}_${factoryId}`;
  ...
  async function submitAdjust(e?: React.FormEvent) {
    e?.preventDefault();
    const rec = adjustRecord as Partial<PackagingStock> & { amount?: number };
    if (!rec.packagingItemId || !rec.factoryId)
    ...
    try {
      if (rec._id) {
        ...
        await api.put(`/packaging-stocks/${rec._id}`, {
          quantity: newQty,
          batch: rec.batch,
        ...
        // create new
        await api.post("/packaging-stocks", {
          packagingItemId: rec.packagingItemId,
  ...
  return (
    <div className="space-y-6 p-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Packaging Materials</h2>
          <p className="text-sm text-muted-foreground">
            Manage packaging items and per-factory stocks
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            onClick={async () => {
              try {
                const res = await api.get("/packaging-items/export", {
                  responseType: "blob",
                });
                const url = window.URL.createObjectURL(new Blob([res.data]));
                const a = document.createElement("a");
                a.href = url;
                a.download = "packaging_items.csv";
                a.click();
              ...
              {items.length === 0 && (
                <TableRow>
                  <TableCell colSpan={9} className="text-center py-6">
                    {loading ? "Loading..." : "No packaging items found"}
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
        ...
        <DialogContent>
          <form onSubmit={submitForm} className="space-y-4">
            <h3 className="text-lg font-semibold">
              {editing?._id ? "Edit Packaging Item" : "New Packaging Item"}
            </h3>
      ...
      <Dialog open={adjustOpen} onOpenChange={setAdjustOpen}>
        <DialogContent>
          <form onSubmit={submitAdjust} className="space-y-4">
            <h3 className="text-lg font-semibold">Adjust Packaging Stock</h3>
